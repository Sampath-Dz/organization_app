import uuid
from sqlalchemy import Column, String, Integer, TEXT, ForeignKey, UniqueConstraint, Table, inspect
...
class Organization(Base, BaseModel):
    ...
    name = Column(String(256), nullable=False, info=ColumnPermissions.full)
    ...
    __table_args__ = (
        UniqueConstraint("name", "deleted_at", name="uq_organization_name_deleted_at"),
    )
    ...
    def to_schema(self):
        from core_service.core_apis_server.schemas.org import OrganizationResponse
...
class Team(Base, BaseModel):
    ...
    name = Column(String(256), nullable=False, info=ColumnPermissions.full)
    description = Column(TEXT, nullable=True, info=ColumnPermissions.full)
    organization_id = Column(String(36), ForeignKey("organizations.id"), nullable=False, info=ColumnPermissions.create_only)
    parent_id = Column(String(36), ForeignKey("teams.id"), nullable=True, info=ColumnPermissions.create_only)
    ...
    __table_args__ = (
        UniqueConstraint("name", "organization_id", "deleted_at", name="uq_team_name_org_deleted_at"),
    )

    def __init__(self, name=None, description=None, organization_id=None, parent_id=None):
        self.name = name
        self.description = description
...
class Member(Base, BaseModel):
    ...
    team_id = Column(String(36), ForeignKey("teams.id"), nullable=False, info=ColumnPermissions.create_only)
    auth_user_id = Column(String(36), nullable=False, info=ColumnPermissions.create_only)

    team = relationship("Team", back_populates="members")

    __table_args__ = (
        UniqueConstraint("team_id", "auth_user_id", "deleted_at", name="uq_member_team_user_deleted_at"),
    )
